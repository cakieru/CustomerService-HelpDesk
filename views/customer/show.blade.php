...
<div class="max-w-6xl mx-auto py-6">
    ...
    <div class="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm mb-6">
        <div class="text-xs text-gray-400 font-medium mb-1.5 flex items-center space-x-2">
            ...
            <span>•</span>
            <span>Opened {{ $ticket->created_at ? $ticket->created_at->format('M d, Y, g:i A') : 'Recently' }}</span>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <!-- Dynamic Replies Loop (Checks replies, comments, or messages relationships) -->
            @php
                $replies = $ticket->replies ?? $ticket->comments ?? $ticket->messages ?? collect();
            @endphp

            @foreach($replies as $reply)
                <div class="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm {{ isset($reply->user) && $reply->user->is_admin ? 'border-l-4 border-l-indigo-500' : '' }}">
                    <div class="flex items-center justify-between mb-3 pb-3 border-b border-gray-100">
                        <div class="flex items-center space-x-3">
                            <div class="w-8 h-8 rounded-full {{ isset($reply->user) && $reply->user->is_admin ? 'bg-indigo-100 text-indigo-700' : 'bg-gray-100 text-gray-700' }} font-bold text-xs flex items-center justify-center">
                                {{ isset($reply->user) ? substr($reply->user->name, 0, 1) : 'U' }}
                            </div>
                            <h4 class="text-sm font-bold text-gray-900">
                                {{ $reply->user->name ?? 'Support Agent' }}
                                @if(isset($reply->user) && $reply->user->is_admin)
                                    <span class="ml-1 px-1.5 py-0.5 bg-indigo-50 text-indigo-600 rounded text-[10px] font-semibold">Staff</span>
                                @endif
                            </h4>
                        </div>
                        <span class="text-xs text-gray-400">{{ $reply->created_at ? $reply->created_at->diffForHumans() : '' }}</span>
                    </div>
                    <p class="text-sm text-gray-700 leading-relaxed whitespace-pre-wrap">{{ $reply->message ?? $reply->body ?? $reply->content }}</p>
                </div>
            @endforeach

            <!-- Functional Add a Reply Box -->
            <div class="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
                <h3 class="text-sm font-bold text-gray-900 mb-3">Add a reply</h3>
                <!-- Submits securely to your backend reply route -->
                <form action="{{ route('customer.reply', $ticket->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <textarea name="body" rows="4" required placeholder="Add more details, ask a follow-up, or provide additional information..." class="w-full p-3.5 bg-gray-50 border border-gray-200 rounded-xl text-xs text-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:bg-white resize-none mb-3"></textarea>
            ...
            <!-- Empty State Notice if there are no replies yet -->
            @if($replies->isEmpty())
            ...
            <div class="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm">
                ...
                <div class="space-y-2.5 text-xs">
                    ...
                    <div class="flex justify-between py-1 border-b border-gray-50">
                        ...
                        <span class="font-medium text-gray-800">{{ $ticket->created_at ? $ticket->created_at->format('M d, Y') : 'N/A' }}</span>
                    </div>
                    <div class="flex justify-between py-1">
                        <span class="text-gray-400">Last Updated</span>
                        <span class="font-medium text-gray-800">{{ $ticket->updated_at ? $ticket->updated_at->format('M d, Y') : 'N/A' }}</span>
                    </div>
                </div>
            </div>
